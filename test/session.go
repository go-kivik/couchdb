...
func session(ctx *kt.Context) {
	chttpAdmin, err := chttp.NewWithClient(&http.Client{}, ctx.Admin.DSN(), nil)
	if err != nil {
		ctx.Fatalf("chttp.Admin failed: %s", err)
	}
	chttpNoAuth, err := chttp.NewWithClient(&http.Client{}, ctx.NoAuth.DSN(), nil)
	if err != nil {
...
func testCreateSession(ctx *kt.Context, client *chttp.Client) {
	...
	client, _ = chttp.NewWithClient(&http.Client{}, client.DSN(), nil)
